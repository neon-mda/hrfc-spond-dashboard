...
def render_card(results, card_title, subtitle_suffix=""):
    ...
    card_html = (
        ...
        '.card-wrapper { display: inline-block; position: relative; max-width: 100%; padding-top: 4px; }'
        '.card {'
        '  background-color: #1C0304;'
        '  border: 1px solid rgba(130, 28, 52, 0.5);'
        '  border-radius: 16px;'
        '  padding: 18px 16px;'
        '  width: fit-content;'
        '  max-width: 100%;'
        ...
        '      <div>'
        f'        <div style="color: #FFE602; font-size: 16px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; line-height: 1.2;">{card_title}</div>'
        f'        <div style="color: #F3C5CE; font-size: 11px; font-weight: 500; margin-top: 4px;">{subtitle}</div>'
        '      </div>'
        f'      <div class="logos-wrapper">{logos_html}</div>'
        '    </div>'
        ...
        '    clippedCanvas.toBlob(async (blob) => {'
        '      if (!blob) {'
